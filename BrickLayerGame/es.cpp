...
/**********************************************************************************************/
		...
		void  DrawSphere(){
		for(float i=0 ; i<361; i+=45) {for(float j=0; j<361; j+=45){
		float theta2 = Deg2rad(i);
		float theta = Deg2rad(j);
		DrawTriangle(c, d,  c+9 * cos(theta),
				 d+9 * sin(theta), c + 9 * cos(theta2),
				d + 9 * sin(theta2), colors[4]);}}}
...
int l;
	...
	for(int midy = 200; midy <600; midy +=50){if(midy%2==0){l=1;}else{l=2;}
	
		
		DrawTriangle( midx , midy , midx+90 , midy , midx , midy+50 , colors[l]);
		DrawTriangle( midx+90, midy ,midx , midy +50,midx +90, midy +50, colors[l]);midy=midy+1;l=l+1;}}}
...
int t1=685,t2=685,t3=685,s1=785,s2=785,s3=785;
	void Timer(int m) {
		...
		Display();
		 if(d<t1  && c<s1){d=d+sy;c=c+sx;}else if(d<=0){exit(1);}
